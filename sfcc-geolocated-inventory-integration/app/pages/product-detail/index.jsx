...
const ProductDetail = ({category, product, isLoading}) => {
    ...
    const {selectedStore} = useContext(StoresContext)

    useEffect(() => {
        const locationPathObject = location.pathname.split('/')
        if (locationPathObject.length > 3 && selectedStore) {
            const locationInventoryId = locationPathObject[3]
            const replacedLocationPathname = location.pathname.replace(locationInventoryId, selectedStore.inventoryId)
            const updatedUrl = rebuildPathWithParams(`${replacedLocationPathname}${location.search}`, {
...
ProductDetail.getProps = async ({res, params, location, api}) => {
    const {productId, inventoryListId} = params
    ...
    product = await api.shopperProducts.getProduct({
        parameters: {
            ...
            allImages: true,
            inventoryIds: inventoryListId
        }
    })

    if (product.inventories && product.inventories.length > 0) {
        product.inventory = product.inventories[0]
    }
